...
def paginate(doctype, page=0):
    ...
    if(page):
        page = int(page)
        properties = frappe.db.sql(query + f" LIMIT {(page*4)-4}, 4", as_dict=True)
        next_set = frappe.db.sql(query + f" LIMIT {(page*4)}, 4", as_dict=True)  # to find if new set values exist or page next exists
        if(next_set):
